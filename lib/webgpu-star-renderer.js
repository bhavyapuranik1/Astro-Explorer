...
(function (window) {
  ...
  class WebGPUStarRenderer {
    ...
    updateInstanceBuffer(instanceData, instanceCount) {
      ...
      const byteLength = instanceCount * 7 * 4; // 7 floats per instance
      if (!this.vertexBuffer || this.bufferCapacity < instanceCount) {
        if (this.vertexBuffer) this.vertexBuffer.destroy();
        this.bufferCapacity = Math.max(instanceCount, (this.bufferCapacity || 0) * 2, 1024);
        this.vertexBuffer = this.device.createBuffer({
          size: this.bufferCapacity * 7 * 4,
      ...
      this.starCount = instanceCount;
    }

    drawInstanced(renderPassEncoder) {
      if (!this.ready || !this.pipeline || !this.vertexBuffer || this.starCount <= 0) return;
      renderPassEncoder.setPipeline(this.pipeline);
      renderPassEncoder.setVertexBuffer(0, this.vertexBuffer);
      renderPassEncoder.draw(6 * this.starCount, 1, 0, 0);
    }
