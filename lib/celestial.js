...
canvas.node().style.height =
  ...
  function actualRedraw() {  
    ...
    if (cfg.mw.show) { 
      ...
      selectCached(parentElement + " .mw").each(function(d) { 
        ...
        context.globalAlpha *= overlayDimmer * mwBright;
        map(d); 
        context.fill(); 
      });
  ...
  function starColor(d) {
    ...
    if (sat <= 1.0) {
      ...
      // Saturation > 1.0 -> High-impact vivid spectral boost
      var factor = sat * 1.8;
      var avg = (r + g + b) / 3;
      r = Math.max(0, Math.min(255, Math.round(avg + (r - avg) * factor)));
      g = Math.max(0, Math.min(255, Math.round(avg + (g - avg) * factor)));
      b = Math.max(0, Math.min(255, Math.round(avg + (b - avg) * factor)));
    }
