...
canvas.node().style.height =
  ...
  function actualRedraw() {  
    ...
    if (cfg.stars.show) { 
      ...
      // Direct star magnitude limit driven 100% by cfg.stars.limit (Star Magnitude slider)
      var skyLimit = (typeof cfg.stars.limit !== "undefined") ? cfg.stars.limit : 4.0;
      ...
      selectCached(parentElement + " .star").each(function(d) {
        ...
        if (typeof skySettings !== "undefined" && skySettings.enableTwinkling) {
          ...
          var wave1 = Math.sin(nowTime * speed + (starId % 37));
          var wave2 = Math.sin(nowTime * speed * 2.3 + (starId % 73));
          twinkleFactor = 1.0 + ((wave1 + wave2) / 2) * intensity;
        ...
        var starAlpha = Math.max(0.40, Math.min(1.0, (1.0 - normMag * 0.45) * twinkleFactor * lightPollutionDimming));
        var cHex = starColor(d);

        if (d.properties.mag < 1.5) {
          brightStars.push({ pt: pt, r: Math.max(0.8, r), alpha: starAlpha, color: cHex, mag: d.properties.mag });
        } else {
          // Quantize alpha to 0.1 steps for batching
          var qAlpha = (Math.round(starAlpha * 10) / 10).toFixed(1);
          var batchKey = cHex + "_" + qAlpha;
          if (!starBatches[batchKey]) {
            starBatches[batchKey] = { color: cHex, alpha: parseFloat(qAlpha), stars: [] };
          }
          starBatches[batchKey].stars.push({ x: pt[0], y: pt[1], r: Math.max(0.8, r) });
        }
      });

      // Render Batched Stars (99%+ of stars drawn in ~6-8 combined canvas paths!)
      for (var bKey in starBatches) {
        var batch = starBatches[bKey];
        context.save();
        context.globalAlpha = batch.alpha;
        context.fillStyle = batch.color;
        context.beginPath();
        for (var s = 0; s < batch.stars.length; s++) {
          var st = batch.stars[s];
          context.moveTo(st.x + st.r, st.y);
          context.arc(st.x, st.y, st.r, 0, 2 * Math.PI);
        }
        context.fill();
        context.restore();
      }

      // Render Bright Stars with Shadow Blur Glow
      for (var b = 0; b < brightStars.length; b++) {
        var bs = brightStars[b];
        context.save();
        context.globalAlpha = bs.alpha;
        context.fillStyle = bs.color;
        context.shadowColor = bs.color;
        context.shadowBlur = Math.max(2, (1.5 - bs.mag) * 3);
        context.beginPath();
        context.arc(bs.pt[0], bs.pt[1], bs.r, 0, 2 * Math.PI);
        context.fill();
        context.restore();
      }
    }
