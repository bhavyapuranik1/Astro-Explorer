  ...
  function actualRedraw() {  
    ...
    if (cfg.mw.show) { 
      ...
      if (finalMwBright > 0.05) {
          // Because d3-celestial paths overlap extensively (creating additive white blobs when opacity/blur is applied globally),
          // we use a much cleaner approach: a very subtle single global fill.
          context.save();
          context.globalAlpha = overlayDimmer * finalMwBright * 0.035;

          selectCached(parentElement + " .mw").each(function(d) {
            context.beginPath();
            map(d);
            // Simple flat fill. Removing shadowBlur entirely to prevent GPU overlap artifacts.
            // A very low alpha value naturally simulates faint dust lanes where the polygons overlap.
            context.fillStyle = cfg.mw.style.fill || "#ffffff";
            context.fill();
          });
          context.restore();
      }
