    ...
    if (cfg.stars.show) { 
      ...
      selectCached(parentElement + " .star").each(function(d) {
        ...
        if (alt > 0) {
          ...
          extMag = 999; // below horizon, invisible
        }

        var effectiveMag = d.properties.mag + extMag;

        if (clip(coords) && effectiveMag <= skyLimit && alt > 0) {
          totalStarsPassed++;
          var pt = mapProjection(coords),
              r = starSize(d); // Using calculated star size directly, clamping happens in starSize
