  ...
  function actualRedraw() {  
    ...
    if (cfg.mw.show) { 
      ...
      if (finalMwBright > 0.05) {
          // Realistic multi-depth astronomical Milky Way rendering (Stellarium style)
          var mwLayerStyles = {
          ...
          selectCached(parentElement + " .mw").each(function(d) {
            ...
            context.save();
            context.globalAlpha = Math.min(1.0, overlayDimmer * finalMwBright * st.alpha);
            context.fillStyle = st.fill;
            context.strokeStyle = st.stroke;
            context.lineWidth = st.lineWidth;
            context.shadowColor = st.shadowColor;
            context.shadowBlur = st.shadowBlur;
