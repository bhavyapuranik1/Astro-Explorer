...
canvas.node().style.height =
  ...
  function actualRedraw() {  
    ...
    if (cfg.mw.show) { 
      ...
      var finalMwBright = Math.max(0, mwBright - (moonWashout * 0.7)); // Moonlight washes out MW

      selectCached(parentElement + " .mw").each(function(d) { 
        setStyle(cfg.mw.style); 
        context.globalAlpha *= overlayDimmer * finalMwBright;
        // Improve milky way by applying a slight blur for realism if supported (mostly a visual hack)
        context.shadowColor = cfg.mw.style.fill;
        context.shadowBlur = 10;
        map(d); 
        context.fill(); 
        context.shadowBlur = 0; // reset
      });
      // mw-outside background masking disabled to prevent star clipping and boundary cutoff
      /*
      if (cfg.transform !== "supergalactic" && cfg.background.opacity > 0.95 && overlayDimmer >= 0.1)
        selectCached(parentElement + " .mwbg").each(function(d) { 
