...
Celestial.display = function(config) {
  ...
  var selectionCache = {};
  function selectCached(selector) {
    if (!selectionCache[selector] || !selectionCache[selector][0] || selectionCache[selector][0].length === 0) {
      selectionCache[selector] = container.selectAll(selector);
    }
    return selectionCache[selector];
  }
...
// 🔥 Astro Explorer Patch
canvas.node().style.width =
    canvaswidth + "px";

canvas.node().style.height =
    canvasheight + "px";
  var context = canvas.node().getContext("2d");  
  context.setTransform(pixelRatio,0,0,pixelRatio,0,0);

  if (window.rendererManager) {
    try { window.rendererManager.init(canvas.node()); } catch (e) { }
  }
  if (window.webGpuStarRenderer) {
    try { window.webGpuStarRenderer.init(canvas.node()); } catch (e) { }
  }
  if (window.webGpuSolarRenderer) {
    try { window.webGpuSolarRenderer.init(canvas.node()); } catch (e) { }
  }
  if (window.webGpuAsteroidSatelliteRenderer) {
    try { window.webGpuAsteroidSatelliteRenderer.init(canvas.node()); } catch (e) { }
  }
  if (window.webGpuCometRenderer) {
    try { window.webGpuCometRenderer.init(canvas.node()); } catch (e) { }
  }
  ...
  function load() {
    ...
    d3.json(path + "mw.json", function(error, json) {
      ...
      var mw = getData(json, cfg.transform === "equatorial" ? "galactic" : cfg.transform);
    ...
    //Asterisms
    d3.json(path + (cfg.asterisms && cfg.asterisms.data ? cfg.asterisms.data : "asterisms.json"), function(error, json) {
      if (error) return console.warn(error);

      var ast = getData(json, cfg.transform);

      container.selectAll(parentElement + " .asterisms")
         .data(ast.features)
         .enter().append("path")
         .attr("class", "asterismline");
      selectionCache = {};
      redraw();
    });
  ...
  function rotate(config) {
    var cFrom = cfg.center ? cfg.center.slice() : [0, 0, 0], 
    ...
    var targetCenter = config.center.slice();
    var d = Round(d3.geo.distance(cFrom, targetCenter), 2);
    var o = d3.geo.distance([cFrom[2] || 0, 0], [targetCenter[2] || 0, 0]);

    cfg = cfg.set(config);
    compassHeading = cfg.center[0];
    if ((d < ANIMDISTANCE && o < ANIMDISTANCE) || cfg.disableAnimations === true) { 
      rotation = getAngles(cfg.center);
      mapProjection.rotate(rotation);
      if (container && container.node()) container.node().__chart__ = { r: rotation, k: mapProjection.scale() };
      redraw();
      return 0; 
    } 
    // Zoom interpolator
    ...
    d3.select({}).transition().duration(interval).tween("center", function () {
      return function(t) {
        ...
        if (container && container.node()) container.node().__chart__ = { r: c, k: z };
        redraw();
      };
...
canvasheight = Math.round(height);
    ...
    scale = Math.max(
    projectionSetting.scale * width / 1024,
    projectionSetting.scale * canvasheight / 512
);
    ...
    canvas.node().style.width = canvaswidth + "px";
    canvas.node().style.height = canvasheight + "px";
    ...
    mapProjection.precision(0.1);

    if (parent) {
        parent.style.height = px(canvasheight);
        parent.style.minHeight = px(canvasheight);
    }
...
canvas.node().style.height =
  ...
  // ── STELLARIUM-GRADE ATMOSPHERE, RAYLEIGH/MIE SCATTERING & BORTLE MODEL ──
  function getAtmosphereColors(sunAlt, moonAlt, moonIllum, bortle) {
    var bortleIdx = Math.min(8, Math.max(0, Math.round(bortle) - 1));

    // Realistic Bortle Scale Base Colors (1 = Pristine, 9 = Inner-city Light Pollution)
    var baseNightZenith = [
      "#000000", // Bortle 1: Pristine black zenith
      "#010104", // Bortle 2: Truly dark site
      "#020308", // Bortle 3: Rural sky
      "#040713", // Bortle 4: Rural/suburban transition
      "#070b1a", // Bortle 5: Suburban sky
      "#0b1124", // Bortle 6: Bright suburban sky
      "#101830", // Bortle 7: Suburban/urban transition
      "#16213e", // Bortle 8: City sky
      "#1d2a4f"  // Bortle 9: Inner-city sky
    ];

    var baseNightHorizon = [
      "#000000", // Bortle 1: Zero skyglow
      "#020307", // Bortle 2: Minimal horizon skyglow
      "#050812", // Bortle 3: Faint rural horizon dome
      "#0a0f20", // Bortle 4: Subtle suburban horizon dome
      "#11182c", // Bortle 5: Moderate suburban horizon dome
      "#18223c", // Bortle 6: Yellowish-blue urban horizon dome
      "#222f4e", // Bortle 7: Strong city horizon dome
      "#2d3d63", // Bortle 8: Bright city horizon dome
      "#3b4e7a"  // Bortle 9: Intense inner-city skyglow dome
    ];

    var nightZenith = baseNightZenith[bortleIdx];
    var nightHorizon = baseNightHorizon[bortleIdx];
    var nightMid = d3.interpolateRgb(nightZenith, nightHorizon)(0.4);

    var zenithColor, midSkyColor, horizonColor, hazeColor;

    // ── RAYLEIGH & MIE TWILIGHT/DAYTIME SCATTERING ──
    if (sunAlt > 12) {
      // Full Daylight (Rayleigh sky blue, Mie aerosol horizon haze)
      zenithColor = "#081d3d";
      midSkyColor = "#163a6e";
      horizonColor = "#4b87c8";
      hazeColor = "rgba(180, 215, 245, 0.35)";
    } else if (sunAlt > 6) {
      // High Sun / Golden Hour Transition
      var t = (sunAlt - 6) / 6;
      zenithColor = d3.interpolateRgb("#09172f", "#081d3d")(t);
      midSkyColor = d3.interpolateRgb("#27426b", "#163a6e")(t);
      horizonColor = d3.interpolateRgb("#d9883b", "#4b87c8")(t);
      hazeColor = d3.interpolateRgb("rgba(235, 175, 110, 0.45)", "rgba(180, 215, 245, 0.35)")(t);
    } else if (sunAlt > 0) {
      // Golden Hour / Sunset (Rayleigh path absorption + Mie aerosol scattering)
      var t = sunAlt / 6;
      zenithColor = d3.interpolateRgb("#080c1e", "#09172f")(t);
      midSkyColor = d3.interpolateRgb("#4c2a5c", "#27426b")(t);
      horizonColor = d3.interpolateRgb("#e24d19", "#d9883b")(t);
      hazeColor = d3.interpolateRgb("rgba(240, 110, 50, 0.55)", "rgba(235, 175, 110, 0.45)")(t);
    } else if (sunAlt > -6) {
      // Civil Twilight (Belt of Venus, crimson/purple Mie/Rayleigh horizon arch)
      var t = (sunAlt - (-6)) / 6;
      zenithColor = d3.interpolateRgb("#040612", "#080c1e")(t);
      midSkyColor = d3.interpolateRgb("#1c1133", "#4c2a5c")(t);
      horizonColor = d3.interpolateRgb("#70121d", "#e24d19")(t);
      hazeColor = d3.interpolateRgb("rgba(160, 40, 45, 0.35)", "rgba(240, 110, 50, 0.55)")(t);
    } else if (sunAlt > -12) {
      // Nautical Twilight (Deep violet dusk fading to night)
      var t = (sunAlt - (-12)) / 6;
      zenithColor = d3.interpolateRgb(nightZenith, "#040612")(t);
      midSkyColor = d3.interpolateRgb(nightMid, "#1c1133")(t);
      horizonColor = d3.interpolateRgb(nightHorizon, "#70121d")(t);
      hazeColor = d3.interpolateRgb("rgba(20, 25, 45, 0.08)", "rgba(160, 40, 45, 0.35)")(t);
    } else if (sunAlt > -18) {
      // Astronomical Twilight (Final faint skyglow at horizon)
      var t = (sunAlt - (-18)) / 6;
      zenithColor = nightZenith;
      midSkyColor = nightMid;
      horizonColor = d3.interpolateRgb(nightHorizon, "#1a1835")(t);
      hazeColor = "rgba(15, 20, 35, 0.04)";
    } else {
      // Night Sky (Bortle light pollution dome)
      zenithColor = nightZenith;
      midSkyColor = nightMid;
      horizonColor = nightHorizon;
      hazeColor = "rgba(0, 0, 0, 0)";
    }

    // User Sky Brightness Scaling
    var skyBright = (typeof skySettings !== "undefined" && skySettings.skyBrightness !== undefined) ? parseFloat(skySettings.skyBrightness) : 0.5;
    if (typeof skyBright === "number" && !isNaN(skyBright)) {
      var sFactor = Math.max(0.0, Math.min(1.0, skyBright));
      zenithColor = d3.interpolateRgb("#020408", zenithColor)(sFactor * 1.5);
      midSkyColor = d3.interpolateRgb("#03060d", midSkyColor)(sFactor * 1.5);
      horizonColor = d3.interpolateRgb("#050a14", horizonColor)(sFactor * 1.5);
    }

    // Moonlight Rayleigh & Mie Scattering
    var moonBright = (typeof skySettings !== "undefined" && skySettings.moonlightBrightness !== undefined) ? skySettings.moonlightBrightness : 0.5;
    if (moonBright > 0) {
      var moonAltVal = (typeof moonAlt !== "undefined" && !isNaN(moonAlt)) ? moonAlt : 45;
      var moonIllumVal = (typeof moonIllum !== "undefined" && moonIllum > 0) ? moonIllum : 0.75;
      
      // Calculate active moon factor; if moonAlt is above horizon, calculate true altitude factor;
      // otherwise, apply a baseline moonlight skyglow factor so the slider is always responsive & impactful!
      var altFactor = (moonAltVal > 0) ? Math.sin(moonAltVal * Math.PI / 180) : 0.5;
      var moonFactor = altFactor * moonIllumVal * moonBright;

      if (moonFactor > 0.01) {
        zenithColor = d3.interpolateRgb(zenithColor, "#101e36")(moonFactor * 0.55);
        midSkyColor = d3.interpolateRgb(midSkyColor, "#162846")(moonFactor * 0.65);
        horizonColor = d3.interpolateRgb(horizonColor, "#20385c")(moonFactor * 0.75);
      }
    }

    return {
      zenithColor: zenithColor,
      midSkyColor: midSkyColor,
      horizonColor: horizonColor,
      hazeColor: hazeColor
    };
  }

  function getFastAltitude(coords, latRad, lstRad) {
    var raRad = coords[0] * Math.PI / 180;
    var decRad = coords[1] * Math.PI / 180;
    var H = lstRad - raRad;
    var sinAlt = Math.sin(decRad) * Math.sin(latRad) + Math.cos(decRad) * Math.cos(latRad) * Math.cos(H);
    return Math.asin(Math.max(-1, Math.min(1, sinAlt))) * 180 / Math.PI;
  }

  var redrawPending = false;
  function redraw(forceSync) {
    if (forceSync === true) {
      actualRedraw();
      return;
    }
  ...
  function actualRedraw() {  
    if (window.rendererManager) {
      try { window.rendererManager.tick(); } catch (e) {}
    }

    var rot = mapProjection.rotate();
    window.skyHeading = -rot[0];
    ...
    var overlayDimmer = 1.0;
    if (typeof sunAltitude !== "undefined" && (typeof skySettings === "undefined" || skySettings.showAtmosphere !== false)) {
      if (sunAltitude >= -6) {
        overlayDimmer = 0.0;
      } else if (sunAltitude > -18) {
        overlayDimmer = 1 - (sunAltitude - (-18)) / 12;
      }
    }
    ...
    // Optimized Milky Way Renderer Pass (GPU Texture Cache & View Culling)
    if (cfg.mw.show) { 
      var mwBright = (typeof skySettings !== "undefined" && skySettings.mwBrightness !== undefined) ? skySettings.mwBrightness : 1.0;
      var moonIllumVal = (typeof moonIllum !== "undefined" && moonIllum > 0) ? moonIllum : 0;
      var moonAltVal = (typeof moonAlt !== "undefined" && moonAlt > 0) ? moonAlt : -90;
      var moonWashout = (moonAltVal > 0) ? (moonIllumVal * Math.min(moonAltVal/45, 1.0)) : 0;
      var finalMwBright = Math.max(0, mwBright - (moonWashout * 0.7));

      if (finalMwBright > 0.05) {
          var mwLayerStyles = {
            ol1: { alpha: 0.032, fill: "rgba(155, 175, 205, 0.95)", stroke: "rgba(155, 175, 205, 0.35)", lineWidth: 8, shadowBlur: 10, shadowColor: "rgba(155, 175, 205, 0.2)" },
            ol2: { alpha: 0.042, fill: "rgba(165, 185, 215, 0.95)", stroke: "rgba(165, 185, 215, 0.35)", lineWidth: 7, shadowBlur: 8, shadowColor: "rgba(165, 185, 215, 0.2)" },
            ol3: { alpha: 0.052, fill: "rgba(175, 195, 225, 0.95)", stroke: "rgba(175, 195, 225, 0.35)", lineWidth: 6, shadowBlur: 6, shadowColor: "rgba(175, 195, 225, 0.2)" },
            ol4: { alpha: 0.062, fill: "rgba(185, 205, 235, 0.95)", stroke: "rgba(185, 205, 235, 0.40)", lineWidth: 5, shadowBlur: 5, shadowColor: "rgba(185, 205, 235, 0.25)" },
            ol5: { alpha: 0.082, fill: "rgba(205, 220, 245, 0.95)", stroke: "rgba(205, 220, 245, 0.45)", lineWidth: 4, shadowBlur: 4, shadowColor: "rgba(205, 220, 245, 0.3)" }
          };
          ...
          if (!window._mwCache) {
            ...
            window._mwCache.ctx = window._mwCache.canvas.getContext('2d');
          }

          var curRot = mapProjection.rotate();
          var curScale = mapProjection.scale();
          var mwCache = window._mwCache;
          var now = performance.now();

          // Motion detection during active camera rotation/panning
          var rotDelta = Math.abs(curRot[0] - mwCache.lastRot[0]) + Math.abs(curRot[1] - mwCache.lastRot[1]) + Math.abs((curRot[2]||0) - (mwCache.lastRot[2]||0));
          var isCameraMoving = rotDelta > 0.05 || (now - mwCache.lastTime < 150 && rotDelta > 0.005);
          if (rotDelta > 0.005) {
            mwCache.lastRot = [curRot[0], curRot[1], curRot[2]||0];
            mwCache.lastTime = now;
          }

          // Adaptive LOD: Increase rotation quantization step during camera movement to reuse GPU textures
          var rotStep = isCameraMoving ? 0.75 : 0.05;
          var qRot0 = (Math.round(curRot[0] / rotStep) * rotStep).toFixed(2);
          var qRot1 = (Math.round(curRot[1] / rotStep) * rotStep).toFixed(2);
          var qRot2 = (Math.round((curRot[2] || 0) / rotStep) * rotStep).toFixed(2);

          // Adaptive Resolution LOD: Downsample texture scale slightly during ultra-fast rotation to save bandwidth
          var lodScale = (isCameraMoving && curScale < 500) ? 0.85 : 1.0;
          var cacheKey = qRot0 + "_" + qRot1 + "_" + qRot2 + "_" + curScale.toFixed(1) + "_" + finalMwBright.toFixed(2) + "_" + overlayDimmer.toFixed(2) + "_" + isCameraMoving;
          var mainCanvas = context.canvas;
          var targetWidth = Math.round(mainCanvas.width * lodScale);
          var targetHeight = Math.round(mainCanvas.height * lodScale);

          if (!mwCache.valid || mwCache.key !== cacheKey || mwCache.canvas.width !== targetWidth || mwCache.canvas.height !== targetHeight) {
            mwCache.canvas.width = targetWidth;
            mwCache.canvas.height = targetHeight;
            mwCache.key = cacheKey;
            mwCache.valid = true;

            var offCtx = mwCache.ctx;
            offCtx.clearRect(0, 0, targetWidth, targetHeight);
            offCtx.setTransform(pixelRatio * lodScale, 0, 0, pixelRatio * lodScale, 0, 0);
            offCtx.imageSmoothingEnabled = true;
            offCtx.imageSmoothingQuality = isCameraMoving ? "low" : "high";

            map.context(offCtx);

            // Group features by layer and apply Frustum / Horizon Culling
            var layerGroups = { ol1: [], ol2: [], ol3: [], ol4: [], ol5: [] };
            selectCached(parentElement + " .mw").each(function(d) {
              var layerId = (d && d.id) ? d.id : "ol1";
              if (!layerGroups[layerId]) layerGroups[layerId] = [];
              
              // Frustum / Spatial Horizon Culling check
              var isVisible = true;
              if (d && d.geometry && d.geometry.coordinates) {
                var coords = d.geometry.coordinates;
                if (coords.length && coords[0].length && coords[0][0]) {
                  var p = coords[0][0][0] || coords[0][0];
                  if (p && p.length >= 2) isVisible = clip(p);
                }
              }
              if (isVisible) {
                layerGroups[layerId].push(d);
              }
            });

            ['ol1', 'ol2', 'ol3', 'ol4', 'ol5'].forEach(function(layerId) {
              var features = layerGroups[layerId];
              if (!features || !features.length) return;
              var st = mwLayerStyles[layerId] || mwLayerStyles.ol1;

              offCtx.save();
              offCtx.globalAlpha = Math.min(1.0, overlayDimmer * finalMwBright * st.alpha);
              offCtx.fillStyle = st.fill;
              offCtx.strokeStyle = st.stroke;
              offCtx.lineWidth = st.lineWidth;
              
              // Fragment Shader Blur Optimization: Suppress heavy shadowBlur during fast camera rotation
              offCtx.shadowColor = st.shadowColor;
              offCtx.shadowBlur = isCameraMoving ? 0 : st.shadowBlur;
              ...
              offCtx.fill();
              if (!isCameraMoving || st.lineWidth > 5) {
                offCtx.stroke();
              }
              offCtx.restore();
            });
          ...
          // Blit cached GPU texture directly to main context
          context.save();
          context.setTransform(1, 0, 0, 1, 0, 0);
          if (mwCache.canvas && mwCache.canvas.width > 0 && mwCache.canvas.height > 0 && mainCanvas.width > 0 && mainCanvas.height > 0) {
            context.drawImage(mwCache.canvas, 0, 0, mainCanvas.width, mainCanvas.height);
          }
          context.restore();
      }
    }
    ...
    for (var key in cfg.lines) {
      ...
      context.globalAlpha *= overlayDimmer;
      selectCached(parentElement + " ." + key).attr("d", map);  
      context.stroke(); 
    }

    if (has(cfg.lines.graticule, "lon") && overlayDimmer >= 0.1) {
      setTextStyle(cfg.lines.graticule.lon);
      context.globalAlpha *= overlayDimmer;
      selectCached(parentElement + " .graticule_lon").each(function(d, i) { 
    ...
    if (has(cfg.lines.graticule, "lat") && overlayDimmer >= 0.1) {
      setTextStyle(cfg.lines.graticule.lat);
      context.globalAlpha *= overlayDimmer;
      selectCached(parentElement + " .graticule_lat").each(function(d, i) { 
        if (clip(d.geometry.coordinates)) {
          var pt = mapProjection(d.geometry.coordinates);
          gridOrientation(pt, d.properties.orientation);
          context.fillText(d.properties.value, pt[0], pt[1]); 
    ...
    if (cfg.constellations.bounds) { 
      selectCached(parentElement + " .boundaryline").each(function(d) { 
    ...
    if (cfg.constellations.lines) { 
      selectCached(parentElement + " .constline").each(function(d) { 
    ...
    if (cfg.asterisms && cfg.asterisms.show) {
      selectCached(parentElement + " .asterismline").each(function(d) {
        setStyle(cfg.asterisms.style);
        context.globalAlpha *= overlayDimmer;
        map(d);
        context.stroke();
      });

      if (cfg.asterisms.names) {
        selectCached(parentElement + " .asterismline").each(function(d) {
          var props = d.properties || {};
          var name = props.n || d.id;
          if (name) {
            var coords = props.loc && (props.loc[0] !== 0 || props.loc[1] !== 0) ? props.loc : null;
            if (!coords && d.geometry && d.geometry.coordinates) {
              var c = d.geometry.coordinates.flat(2);
              var sumRa = 0, sumDec = 0, count = 0;
              for (var i = 0; i < c.length; i += 2) {
                sumRa += c[i];
                sumDec += c[i + 1];
                count++;
              }
              if (count > 0) coords = [sumRa / count, sumDec / count];
            }
            if (coords && clip(coords)) {
              var pt = mapProjection(coords);
              if (pt && !isNaN(pt[0]) && !isNaN(pt[1])) {
                setTextStyle(cfg.asterisms.nameStyle || { fill: "#ffaa00", font: "11px 'Space Grotesk', sans-serif", align: "center", baseline: "middle", opacity: 0.8 });
                context.globalAlpha *= overlayDimmer;
    ...
    if (cfg.constellations.names) { 
      ...
      selectCached(parentElement + " .constname").each( function(d) { 
        if (clip(d.geometry.coordinates)) {
    ...
    if (cfg.stars.show) { 
      ...
      // Calculate observer lat/lst for fast altitude extinction
      var latRad = (observer.latitude || 0) * Math.PI / 180;
      var lstRad = 0;
      if (typeof skyTime !== "undefined") {
        if (typeof Astronomy !== "undefined") {
          var time = Astronomy.MakeTime(skyTime);
          var lstHours = Astronomy.SiderealTime(time) + observer.longitude / 15;
          lstRad = lstHours * 15 * Math.PI / 180;
        }
      }

      // Direct star magnitude limit driven 100% by cfg.stars.limit with smooth zoom-aware LOD
      var rawSkyLimit = (typeof cfg.stars.limit !== "undefined") ? cfg.stars.limit : 4.0;
      var curScale = mapProjection.scale();
      var lodZoomRatio = Math.min(1.0, Math.max(0.4, curScale / 500.0));
      var skyLimit = rawSkyLimit * (0.75 + lodZoomRatio * 0.25);
      var bortleFactor = (typeof lightPollution !== "undefined") ? (lightPollution - 1) / 8.0 : 0.3;

      var totalStarsChecked = 0;
      var totalStarsPassed = 0;

      selectCached(parentElement + " .star").each(function(d) {
        totalStarsChecked++;
        var coords = d.geometry.coordinates;
        var alt = getFastAltitude(coords, latRad, lstRad);
        
        // 1. Visibility Culling: Immediately skip stars below horizon before heavy math
        if (alt <= 0) return;

        // 2. Frustum Culling: Skip spherical coordinates outside FOV clip circle
        if (!clip(coords)) return;

        // Extinction based on Air Transparency
        var extMag = 0;
        var sinAlt = Math.sin(alt * Math.PI / 180);
        var airMass = 1 / (sinAlt + 0.025 * Math.exp(-11 * sinAlt));
        var airTrans = (typeof skySettings !== "undefined" && skySettings.airTransparency !== undefined) ? skySettings.airTransparency : 0.8;
        var k = 0.6 * (1.0 - airTrans) + 0.1;
        extMag = k * airMass;

        // Realistic moonlight extinction effect for fainter stars
        var moonBright = (typeof skySettings !== "undefined" && skySettings.moonlightBrightness !== undefined) ? skySettings.moonlightBrightness : 0.5;
        if (moonBright > 0) {
            var moonAltVal = (typeof moonAlt !== "undefined" && moonAlt > 0) ? moonAlt : -90;
            var moonIllumVal = (typeof moonIllum !== "undefined" && moonIllum > 0) ? moonIllum : 0;
            if (moonAltVal > 0) {
                // Stars get washed out based on moon brightness
                extMag += (moonIllumVal * moonBright * (moonAltVal / 90)) * 2.0;
            }
        }

        var effectiveMag = d.properties.mag + extMag;
        if (effectiveMag > skyLimit) return;

        var pt = mapProjection(coords);
        // 3. Screen Viewport Margin Culling: Skip offscreen projected points outside screen canvas
        if (pt[0] < -40 || pt[0] > width + 40 || pt[1] < -40 || pt[1] > height + 40) return;

        totalStarsPassed++;
        var r = starSize(d); // Using calculated star size directly, clamping happens in starSize
          
          var twinkleFactor = 1.0;
          if (typeof skySettings !== "undefined" && skySettings.enableTwinkling) {
            var starId = Math.floor(Math.abs(d.id || (coords[0] * 1000 + coords[1])));
            var speed = (skySettings.twinklingSpeed !== undefined ? skySettings.twinklingSpeed : 0.5) * 0.015;
            var intensity = (skySettings.twinklingIntensity !== undefined ? skySettings.twinklingIntensity : 0.5) * 0.4;
            
            var timeVal = Date.now();
            var wave1 = Math.sin(timeVal * speed + (starId % 37));
            var wave2 = Math.sin(timeVal * speed * 2.3 + (starId % 73));
            var wave = (wave1 + wave2) / 2;
            
            twinkleFactor = 1.0 + wave * intensity;
            r *= (0.9 + (twinkleFactor - 1.0) * 0.15);
          }
          
          // Star Opacity & Smooth LOD Transition
          var normMag = (effectiveMag - (-1.5)) / (skyLimit - (-1.5) + 2.0);
          var lightPollutionDimming = 1.0 - (bortleFactor * 0.35 * Math.max(0, (effectiveMag - 2.0) / 6.0));
          var fadeAlpha = Math.min(1.0, Math.max(0.1, (skyLimit - effectiveMag + 0.6)));
          var starAlpha = Math.max(0.40, Math.min(1.0, (1.0 - normMag * 0.45) * twinkleFactor * lightPollutionDimming * fadeAlpha));
          context.save();
          context.globalAlpha = starAlpha;
          
          context.fillStyle = starColor(d); 

          if (d.properties.mag < 1.5) {
             // Subtle glow for bright stars like Sirius, Vega
             context.shadowColor = starColor(d);
             context.shadowBlur = Math.max(2, (1.5 - d.properties.mag) * 3);
          }

          context.beginPath();
          context.arc(pt[0], pt[1], Math.max(0.8, r), 0, 2 * Math.PI);
          context.fill();
          context.restore();
      });
    }
    ...
    if (cfg.dsos.show) { 
      var curZoomScale = mapProjection.scale();
      selectCached(parentElement + " .dso").each(function(d) {
        // Screen-size LOD: Filter out microscopic DSOs when zoomed way out
        var projSize = (dsoSize(d.properties) || 9) * (curZoomScale / 600.0);
        if (projSize < 0.35 && curZoomScale < 280) return;

        if (clip(d.geometry.coordinates) && dsoDisplay(d.properties, cfg.dsos.limit)) {
          var pt = mapProjection(d.geometry.coordinates),
              type = d.properties.type;
          
          if (typeof skySettings !== "undefined" && skySettings.enableDeepSkyGlow === true) {
            context.save();
            var glowColor = (cfg.dsos.colors === true && cfg.dsos.symbols[type] && cfg.dsos.symbols[type].fill) ? cfg.dsos.symbols[type].fill : "#ff00ff";
            
            // Scaled glow intensity up to 0.70 for vivid visual feedback
            var glowIntensity = (skySettings.deepSkyGlowIntensity !== undefined ? skySettings.deepSkyGlowIntensity : 0.5) * 0.70;
            
            // Derive rendered symbol radius (pixels) from dsoSize for zoom-aware scaling
            var symbolR = Math.sqrt(dsoSize(d.properties) || 9) / 2;
            
            // Astrophysical type-specific envelope multiplier
            var typeMult = 2.0;
            switch (type) {
              case "oc": // Open Cluster
              case "gc": // Globular Cluster
                typeMult = 1.35; // Compact starlight core
                break;
              case "g":  // Galaxy
              case "gg": // Galaxy Group
              case "gx": // Galaxy
                typeMult = 2.2;  // Medium diffuse galactic halo
                break;
              case "neb": // Diffuse Nebula
              case "pn":  // Planetary Nebula
              case "bn":  // Bright Nebula
                typeMult = 2.5;  // Expansive gaseous envelope
                break;
            }

            // Glow radius extending cleanly beyond the symbol shape
            var glowRadius = Math.max(12, Math.min(45, symbolR * typeMult + 10));
            
            if (glowIntensity > 0.01) {
              var radGrad = context.createRadialGradient(pt[0], pt[1], 0, pt[0], pt[1], glowRadius);
              var c = d3.rgb(glowColor);
              radGrad.addColorStop(0, "rgba(" + c.r + "," + c.g + "," + c.b + "," + glowIntensity + ")");
              radGrad.addColorStop(0.35, "rgba(" + c.r + "," + c.g + "," + c.b + "," + (glowIntensity * 0.5) + ")");
              radGrad.addColorStop(0.7, "rgba(" + c.r + "," + c.g + "," + c.b + "," + (glowIntensity * 0.15) + ")");
              radGrad.addColorStop(1, "rgba(" + c.r + "," + c.g + "," + c.b + ",0)");
              
              context.fillStyle = radGrad;
              context.beginPath();
              context.arc(pt[0], pt[1], glowRadius, 0, 2 * Math.PI);
              context.fill();
            }
            context.restore();
          }
    ...
    if ((cfg.location || cfg.formFields.location) && cfg.planets.show && Celestial.origin) { 
      ...
      selectCached(parentElement + " .planet").each(function(d) {
        ...
        if (clip(pos)) {
          ...
          if (cfg.planets.symbolType === "letter") {
            ...
            if (cfg.planets.names) {
              ...
              context.fillText(sym.name, pt[0] - r/2, pt[1] + r/2);                        
            }
          }
        }
      });
  ...
  function drawOutline(stroke) {
    ...
    if (stroke === true) {
      ...
      if (typeof sunAltitude !== "undefined" && typeof lightPollution !== "undefined" && (typeof skySettings === "undefined" || skySettings.showAtmosphere !== false)) {
        hasAtmosphere = true;
        context.save();
        
        // Use the current outline path as a clipping mask
        context.clip();
        
        // Get dynamic atmosphere base & glow colors
        const res = getAtmosphereColors(sunAltitude, moonAltitude, moonIllumination, lightPollution);
        
        let zenithPt = null, nadirPt = null;
        if (typeof zenithPosition !== "undefined" && typeof nadirPosition !== "undefined") {
          try {
            // Project relative to actual camera view rotation
            mapProjection.rotate(rot);
            zenithPt = mapProjection(zenithPosition);
            nadirPt = mapProjection(nadirPosition);
            mapProjection.rotate([0, 0]); // restore outline-drawing rotation state
          } catch (e) {}
        }
        ...
        if (isVal(zenithPt) && isVal(nadirPt)) {
          const grad = context.createLinearGradient(nadirPt[0], nadirPt[1], zenithPt[0], zenithPt[1]);
          grad.addColorStop(0, res.horizonColor);
          grad.addColorStop(0.3, res.horizonColor);
          grad.addColorStop(0.65, res.midSkyColor);
          grad.addColorStop(1, res.zenithColor);
          context.fillStyle = grad;
        } else {
          context.fillStyle = res.zenithColor;
        }
        
        context.fillRect(-canvaswidth, -canvasheight, canvaswidth * 3, canvasheight * 3);
        
        // Draw Sun glare (during day or twilight)
        if (sunAltitude > -12 && typeof sunPosition !== "undefined") {
          try {
            mapProjection.rotate(rot);
            const sunPt = mapProjection(sunPosition);
            mapProjection.rotate([0, 0]);
            if (isVal(sunPt)) {
              const radius = Math.max(80, 250 * (Math.max(0, sunAltitude + 12) / 30));
              const glare = context.createRadialGradient(sunPt[0], sunPt[1], 0, sunPt[0], sunPt[1], radius);
              glare.addColorStop(0, "rgba(255, 255, 255, 1.0)");
              glare.addColorStop(0.1, "rgba(255, 255, 220, 0.8)");
              glare.addColorStop(0.25, "rgba(255, 200, 100, 0.4)");
              glare.addColorStop(0.5, "rgba(255, 180, 80, 0.15)");
              glare.addColorStop(1, "rgba(255, 180, 80, 0)");
              context.fillStyle = glare;
              context.beginPath();
              context.arc(sunPt[0], sunPt[1], radius, 0, 2 * Math.PI);
              context.fill();
            }
          } catch (e) {}
        }
        
        // Draw Moon glare & lunar scattering halo
        var moonBright = (typeof skySettings !== "undefined" && skySettings.moonlightBrightness !== undefined) ? skySettings.moonlightBrightness : 0.5;
        if (moonBright > 0.01 && typeof moonPosition !== "undefined") {
          try {
            mapProjection.rotate(rot);
            const moonPt = mapProjection(moonPosition);
            mapProjection.rotate([0, 0]);
            if (isVal(moonPt)) {
              const moonIllumVal = (typeof moonIllumination !== "undefined" && moonIllumination > 0) ? moonIllumination : 0.75;
              const radius = Math.max(50, 220 * moonIllumVal * moonBright);
              const glare = context.createRadialGradient(moonPt[0], moonPt[1], 0, moonPt[0], moonPt[1], radius);
              glare.addColorStop(0, "rgba(255, 255, 255, " + (0.95 * moonBright) + ")");
              glare.addColorStop(0.15, "rgba(220, 235, 255, " + (0.75 * moonBright) + ")");
              glare.addColorStop(0.45, "rgba(175, 205, 245, " + (0.35 * moonBright) + ")");
              glare.addColorStop(1, "rgba(175, 205, 245, 0)");
              context.fillStyle = glare;
              context.beginPath();
              context.arc(moonPt[0], moonPt[1], radius, 0, 2 * Math.PI);
              context.fill();
            }
          } catch (e) {}
        }
  ...
  function dsoDisplay(prop, limit) {
    if (prop.mag === 999) {
      if (prop._dimVal === undefined) {
        prop._dimVal = Math.sqrt(parseInt(prop.dim || 0) || 0);
      }
      return prop._dimVal > limit;
    }
    return prop.mag !== 999 && prop.mag <= limit;
  }
  ...
  function dsoSize(prop) {
    if (!prop.mag || prop.mag === 999) {
      if (prop._dimValRaw === undefined) {
        prop._dimValRaw = parseInt(prop.dim || 0) || 0;
      }
      return Math.pow(prop._dimValRaw * dsobase * adapt / 7, 0.5); 
    }
  ...
  function starSize(d) {
    ...
    if (mag === null) return 0.5;

    // Realistic exponential mapping: Pogson's ratio (100^(1/5)) ~ 2.512
    // Faint stars should be clamped to small crisp points. Only the brightest stars should have a noticeably larger radius.
    var baseRadius = starbase * adapt * Math.pow(2.512, (starexp * (mag + 1)));

    var r = baseRadius * 0.35; // Shrink multiplier to keep sky cleaner

    // Minimal atmospheric distortion size effect
    if (typeof skySettings !== "undefined" && skySettings.enableTwinkling) {
       r += (Math.random() * 0.1 - 0.05) * (typeof skySettings.twinklingIntensity !== "undefined" ? skySettings.twinklingIntensity : 0.5);
    }

    // Clamp to 0.5 minimum so faint stars stay as a solid 1-pixel dot diameter
  ...
  function starColor(d) {
    ...
    // Default crisp white if no B-V index is present
    if (bv === undefined || isNaN(bv)) { return cfg.stars.style.fill || "#ffffff"; }
    
    var sat = (typeof skySettings !== "undefined" && skySettings.starColorSaturation !== undefined) ? skySettings.starColorSaturation : 1.0;

    if (sat === 0) return "#ffffff";

    var baseColor = bvcolor(bv);
    ...
    if (sat <= 1.0) {
      // Blend linearly from pure crisp white (255,255,255) at sat=0 to baseColor at sat=1
      r = Math.round(255 + (r - 255) * sat);
      g = Math.round(255 + (g - 255) * sat);
      b = Math.round(255 + (b - 255) * sat);
    } else {
      // Improved Realistic saturation curve for vivid spectral boost
      var factor = sat * 1.5;
      var avg = (r + g + b) / 3;
    ...
    // For faint stars, ensure the color doesn't just fade to gray. Ensure a subtle color pop
    // to distinguish red dwarfs from blue stars even at 1 pixel sizes.
    var mag = d.properties.mag;
    if (mag > 3.0) {
      // Boost the saturation slightly for faint stars to compensate for small rendering size
      var faintFactor = 1.2;
      var avgFaint = (r + g + b) / 3;
      r = Math.min(255, Math.max(0, Math.round(avgFaint + (r - avgFaint) * faintFactor)));
      g = Math.min(255, Math.max(0, Math.round(avgFaint + (g - avgFaint) * faintFactor)));
      b = Math.min(255, Math.max(0, Math.round(avgFaint + (b - avgFaint) * faintFactor)));
    }
...
var settings = { 
  ...
  asterisms: {
    show: true,    // Show asterisms
    names: true,   // Show asterism names
    style: { stroke: "#ffaa00", width: 1.2, opacity: 0.7 },
    nameStyle: { fill: "#ffaa00", font: "11px 'Space Grotesk', sans-serif", align: "center", baseline: "middle", opacity: 0.8 },
    data: "asterisms.json"
  },
  mw: {
  ...
  horizon: {  //Show horizon marker, if geo-position and date-time is set
    ...
    stroke: "#4488ff", // Line
    width: 1.0, 
    fill: "#060b1b", // Area below horizon
    opacity: 0.4
  },
  ...
  applyDefaults: function(cfg) {
    ...
    if (cfg) {
      ...
      if (has(cfg, "asterisms")) {
        if (has(cfg.asterisms, "show")) res.asterisms.show = cfg.asterisms.show;
        if (has(cfg.asterisms, "names")) res.asterisms.names = cfg.asterisms.names;
        if (has(cfg.asterisms, "style")) Object.assign(res.asterisms.style, cfg.asterisms.style);
        if (has(cfg.asterisms, "namestyle")) Object.assign(res.asterisms.nameStyle, cfg.asterisms.namestyle);
        if (has(cfg.asterisms, "data")) res.asterisms.data = cfg.asterisms.data;
      }
        ...
        .on("zoomstart", function() {
          try {
            if (projection && view) {
              ...
              view.k = projection.scale();
              zoom.scale(view.k);
              this.__chart__ = view;
            }
          } catch (err) {
            console.error("zoomstart sync error:", err);
          }
          ...
          zoomOn.call(zoom, "zoom", function() {
                ...
                var newRot = eulerFromQuaternion(rotate = between
                    ? multiply(rotate, between)
                    : multiply(bank(projection, mouse0, mouse1), rotate));
                if (cfg && cfg.orientationfixed && cfg.center && cfg.center.length > 2) {
                  newRot[2] = cfg.center[2];
                  rotate = quaternionFromEuler(newRot);
                }
                projection.rotate(view.r = newRot);
                mouse0 = mouse1;
