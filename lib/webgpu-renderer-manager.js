  ...
  class RendererManager {
    ...
    async init(canvasElement) {
      ...
      try {
        ...
        // Fetch Exact GPU Model Name
        this.gpuInfo.name = this.detectExactGpuName();

        // WebGPU pipeline active for all 4 subsystems
        this.updateStatusAll('WebGPU');
        console.log("🚀 WebGPU Renderer Manager initialized successfully with GPU:", this.gpuInfo.name);
    ...
    detectExactGpuName() {
      try {
        ...
        if (gl) {
          ...
          if (debugInfo) {
            ...
            if (renderer) {
              renderer = renderer.replace(/^ANGLE\s*\(([^,]+),\s*/i, '').replace(/Direct3D.*$/i, '').replace(/vs_.*$/i, '').trim();
      ...
      if (this.adapter && this.adapter.info) {
        const name = this.adapter.info.device || this.adapter.info.description || this.adapter.info.architecture;
        if (name && name !== 'WebGPU Hardware GPU') return name;
      }
    ...
    updateStatusAll(mode) {
      ...
      this.status.moon = mode;
      this.status.planets = mode;
      this.status.asteroids = mode;
      this.status.satellites = mode;
      this.status.comets = mode;
      if (mode === 'WebGPU') {
        ...
        console.log("⚡ WebGPU Moon Renderer initialized successfully.");
        console.log("⚡ WebGPU Planets Renderer initialized successfully.");
        console.log("⚡ WebGPU Asteroids Renderer initialized successfully.");
        console.log("⚡ WebGPU Satellites Renderer initialized successfully.");
    ...
    toggleOverlay() {
      const nextState = !this.overlayVisible;
      this.setOverlayVisible(nextState);
      if (typeof window.updateSkySettingValue === 'function') {
        window.updateSkySettingValue('showRendererOverlay', nextState);
      }
      return nextState;
    }
    ...
    updateOverlay() {
      ...
      this.overlayElement.innerHTML = `
        <div style="font-weight: bold; font-size: 12px; margin-bottom: 6px; color: ${badgeColor}; display: flex; align-items: center; justify-content: space-between; gap: 8px;">
          ...
          <button id="webgpu-overlay-close-btn" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.2); color: #cbd5e0; border-radius: 4px; padding: 2px 6px; font-size: 12px; cursor: pointer; line-height: 1; transition: all 0.2s;" title="Turn OFF Renderer Status Overlay">✕</button>
        </div>
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px 12px; background: rgba(255,255,255,0.04); padding: 8px; border-radius: 6px; margin-bottom: 8px;">
          ...
          <div>Moon: <b style="color:${this.status.moon === 'WebGPU' ? '#00ffaa' : '#ffbb55'}">${this.status.moon}</b></div>
          <div>Planets: <b style="color:${this.status.planets === 'WebGPU' ? '#00ffaa' : '#ffbb55'}">${this.status.planets}</b></div>
          <div>Asteroids: <b style="color:${this.status.asteroids === 'WebGPU' ? '#00ffaa' : '#ffbb55'}">${this.status.asteroids}</b></div>
          <div>Satellites: <b style="color:${this.status.satellites === 'WebGPU' ? '#00ffaa' : '#ffbb55'}">${this.status.satellites}</b></div>
          <div>Comets: <b style="color:${this.status.comets === 'WebGPU' ? '#00ffaa' : '#ffbb55'}">${this.status.comets}</b></div>
        </div>
      ...
      const closeBtn = this.overlayElement.querySelector('#webgpu-overlay-close-btn');
      if (closeBtn) {
        closeBtn.onclick = (e) => {
          e.stopPropagation();
          this.setOverlayVisible(false);
          if (typeof window.updateSkySettingValue === 'function') {
            window.updateSkySettingValue('showRendererOverlay', false);
          }
        };
      }
    }
