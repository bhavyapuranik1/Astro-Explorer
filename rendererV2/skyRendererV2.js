...
import { MeteorSystem, METEOR_SHOWERS } from './meteorSystem.js';
...
this.milkyWaySystem.setOpacity(this.milkyWayOpacity);
  ...
  /**
   * Returns normalized 3D world direction vector for the Sun.
   * @returns {THREE.Vector3}
   */
  getSunWorldVector() {
    const sunWorldPos = new THREE.Vector3();
    if (this.solarSystem && this.solarSystem.sunMesh) {
      this.solarSystem.sunMesh.getWorldPosition(sunWorldPos);
      if (sunWorldPos.lengthSq() > 0.001) {
        return sunWorldPos.normalize();
      }
    }
    if (this.solarSystem && this.solarSystem.planetCatalog) {
      const match = this.solarSystem.planetCatalog.find(p => p.isSun || p.name === 'Sun');
      if (match && match.position) {
        sunWorldPos.copy(match.position);
        if (this.starSphereGroup) sunWorldPos.applyMatrix4(this.starSphereGroup.matrixWorld);
        return sunWorldPos.normalize();
      }
    }
...
if (this.minorBodiesSystem) {
  ...
  updateLabelsList() {
      ...
      this.solarSystem.planetCatalog.forEach(p => {
        if (p.position) {
          targets.push({
            name: p.name,
            position: p.position,
            type: p.isSun ? 'sun' : (p.isMoon ? 'moon' : 'planet'),
            color: p.isSun ? '#ffffff' : '#ffd700',
            priority: 0,
            ra: p.ra || 0,
            dec: p.dec || 0,
            rawObj: p,
    ...
    if (
      ...
      // METEOR SHOWERS
      // -------------------------
      if (this.showMeteors && Array.isArray(METEOR_SHOWERS)) {
        METEOR_SHOWERS.forEach(shower => {
          const pos = this.celestialToCartesian(shower.ra, shower.dec, this.options.sphereRadius * 0.99);
          targets.push({
  ...
  updateTimeAndObserver(date = new Date(), obs = { latitude: 0, longitude: 0 }) {
    ...
    const sunWorldVec = this.getSunWorldVector();

    let sunAlt = -30;
    let sunAz = 180;

    if (this.atmosphere) {
      const env = this.atmosphere.updateSunPosition(date, obs, sunWorldVec);
      ...
      if (this.starRenderingSystem) {
        if (
          ...
          this.starRenderingSystem.starMaterial.uniforms &&
          this.starRenderingSystem.starMaterial.uniforms.uDaytimeOpacity
        ) {
          this.starRenderingSystem.starMaterial.uniforms.uDaytimeOpacity.value =
            exrDayPresentation ? 0.0 : 1.0;
        }

        if (this.starRenderingSystem.group) {
      ...
      if (
        this.brightStarGlowSystem &&
        this.brightStarGlowSystem.group
      ) {
        this.brightStarGlowSystem.group.visible =
          starVisible;
      }
      ...
      if (this.dsoRenderingSystem) {
        if (this.dsoRenderingSystem.group) {
      ...
      if (this.labelSystem) {
        this.labelSystem.showDSOs = this.showDSOs && !exrDayPresentation;
        this.labelSystem.showStars = this.showStars && !exrDayPresentation;
        this.labelSystem.showPlanets = this.showPlanets;
        this.labelSystem.showSatellites = this.showSatellites && !exrDayPresentation;
      }
      ...
      if (this.milkyWaySystem) {
        if (this.milkyWaySystem.group) {
          this.milkyWaySystem.group.visible = !exrDayPresentation;
        }
    ...
    // ===========================================================
    if (this.solarSystem) {
      const exrDayPresentation = this.showAtmosphere && sunAlt > 0;
      this.solarSystem.updatePositions(date, obs, exrDayPresentation);
    }
    ...
    if (this.meteorSystem) {
      const _sunAlt = this.atmosphere && this.atmosphere.sunAltitude !== undefined ? this.atmosphere.sunAltitude : -30;
      const _exrDay = this.showAtmosphere && _sunAlt > 0;
      this.meteorSystem.setVisible(!!this.showMeteors && !_exrDay);
      this.meteorSystem.update(0.016);
    }
...
setStarTwinklingIntensity(intensity) {
  ...
  setConstellationsVisible(visible) {
    ...
    if (!visible) {
      ...
      if (this.constellationSystem) {
        this.constellationSystem.setConstellationsVisible(false);
        this.constellationSystem.setArtVisible(false);
      }
    } else {
      if (this.constellationSystem) {
        this.constellationSystem.setConstellationsVisible(true);
        this.constellationSystem.setArtVisible(!!this.showConstellationArt);
      }
    }
  }
  ...
  setConstellationArtVisible(visible) {
    ...
    if (visible) {
      ...
      if (this.constellationSystem) {
        this.constellationSystem.setConstellationsVisible(true);
        this.constellationSystem.setArtVisible(true);
      }
    } else {
      if (this.constellationSystem) {
        this.constellationSystem.setArtVisible(false);
  ...
  setMeteorsVisible(visible) {
    this.showMeteors = !!visible;
    if (this.meteorSystem) {
      this.meteorSystem.setVisible(this.showMeteors);
    }
  }
  ...
  setLandscapeVisible(visible) {
    ...
    if (this.landscapeSystem && typeof this.landscapeSystem.setGroundVisible === 'function') {
      this.landscapeSystem.setGroundVisible(visible);
    }
  ...
  setProjectionMode(mode) {
    const m = (mode === 'stereographic' || mode === 'fisheye' || mode === 'planetarium') ? 'stereographic' : 'perspective';
    this.projectionMode = m;
    if (this.labelSystem && typeof this.labelSystem.setProjectionMode === 'function') {
      this.labelSystem.setProjectionMode(m);
    }
  }
...
await this.minorBodiesSystem.init();
    ...
    if (this.minorBodiesSystem && this.minorBodiesSystem.group && this.starSphereGroup) {
      this.starSphereGroup.add(this.minorBodiesSystem.group);

      if (this.minorBodiesSystem.satellitesGroup) {
        this.minorBodiesSystem.satellitesGroup.visible = !!this.showSatellites;
      }
      if (this.minorBodiesSystem.spacecraftGroup) {
        this.minorBodiesSystem.spacecraftGroup.visible = !!this.showSpacecraft;
      }
      if (this.minorBodiesSystem.asteroidsGroup) {
        this.minorBodiesSystem.asteroidsGroup.visible = !!this.showAsteroids;
      }
      if (this.minorBodiesSystem.cometsGroup) {
        this.minorBodiesSystem.cometsGroup.visible = !!this.showComets;
      }
    }

    this.meteorSystem = new MeteorSystem(this.scene);
    if (this.starSphereGroup && this.meteorSystem.group) {
      this.starSphereGroup.add(this.meteorSystem.group);
    }
    this.meteorSystem.setVisible(!!this.showMeteors);
