...
function findSkyObjectUnderCursor(x, y) {
  ...
  for (const type of order) {
    for (const obj of searchObjects) {
      ...
      if (setting && skySettings[setting] === false) continue;
...
function getDsoHitRadius(obj) {
  const cfg = getCelestialRenderSettings();
  const dsobase = (cfg.dsos?.size !== undefined && cfg.dsos.size !== null) ? cfg.dsos.size : (cfg.stars?.size || 7);
  const dsoexp = cfg.dsos?.exponent || cfg.stars?.exponent || -0.28;
  const prop = obj.properties || {};
  let size;
  if (!prop.mag || prop.mag === 999) {
    const dim = parseInt(prop.dim || 0, 10) || 0;
    size = Math.pow(dim * dsobase / 7, 0.5);
  } else {
    size = Math.pow(2 * dsobase - prop.mag, dsoexp);
  }
  const radius = Math.sqrt(Math.max(size, 1)) / 2;
  return radius + 4;
}
...
function getRenderedObjectRadius(obj) {
  ...
  switch (obj.type) {
    case "star":
      return getStarHitRadius(obj.mag);
    case "dso":
      return getDsoHitRadius(obj) + 6;
    case "planet":
      return 12;
    case "comet":
      return 12;
...
function getSkyObjectScreenPoint(obj) {
  ...
  if (typeof Celestial.clip === "function" && !Celestial.clip(pos)) return null;
  try {
...
function getStarHitRadius(mag) {
  const cfg = getCelestialRenderSettings();
  const starBase = cfg.stars?.size || 7;
  const starExp = cfg.stars?.exponent || -0.28;
  const radius = starBase * Math.pow(2.512, starExp * ((typeof mag === "number" ? mag : 0) + 1)) * 0.35;
  return Math.max(radius, 0.5) + 2;
}
...
function getSkyObjectFromLabelTarget(target) {
  ...
  if (label) {
    ...
    if (label.classList.contains("planet-label")) {
      return searchObjects.find(o => o.type === "planet" && ((o.name || "").toLowerCase() === text || (o.id || "").toLowerCase() === text));
    }
    if (label.classList.contains("star-search-label")) {
      return searchObjects.find(o => o.type === "star" && ((o.name || "").toLowerCase() === text || (o.displayName || "").toLowerCase() === text));
    }
    if (label.classList.contains("dso-search-label")) {
      return searchObjects.find(o => o.type === "dso" && ((o.name || "").toLowerCase() === text || (o.displayName || "").toLowerCase() === text));
    }
  }
...
const CONSTELLATION_ART_DEFINITIONS = {
  ori: {
    symbol: "🏹",
    title: "Orion • The Hunter",
    color: "rgba(0, 245, 255, 0.9)",
    lines: [
      [[-25, -40], [-10, -20], [15, -20], [30, -35]],
      [[-10, -20], [-12, 0], [-15, 35]],
      [[15, -20], [12, 0], [18, 38]],
      [[-12, 0], [-2, 0], [8, 0], [-12, 0]],
      [[-25, -40], [-35, -50], [-45, -30], [-42, -10], [-30, 10]],
      [[30, -35], [40, -55], [32, -65]]
    ]
  },
  uma: {
    ...
    color: "rgba(0, 245, 255, 0.9)",
    lines: [
      [[-40, -10], [-25, -15], [-10, -10], [5, 5], [20, 0], [35, 15], [10, 20], [-10, -10]],
      [[35, 15], [50, 10], [65, 20]],
      [[5, 5], [15, 35], [25, 40]],
      [[-25, -15], [-35, 25], [-25, 42]],
      [[-40, -10], [-55, -20], [-70, -15], [-85, -25]]
    ]
  },
  umi: {
    symbol: "🐻‍❄️",
    title: "Ursa Minor • Little Bear",
    color: "rgba(100, 230, 255, 0.9)",
    lines: [
      [[0, 0], [-15, 12], [-30, 8], [-35, -5], [0, 0]],
      [[-35, -5], [-50, -15], [-65, -10], [-85, -20]]
    ]
  },
  leo: {
    symbol: "♌",
    ...
    lines: [
      [[25, -25], [15, -45], [0, -40], [-10, -25], [-5, -10], [15, 10]],
      [[15, 10], [-30, 15], [-50, 5], [-45, -15], [-10, -25]],
      [[-50, 5], [-70, 20]],
      [[15, 10], [10, 30]],
      [[-30, 15], [-35, 35]]
    ]
  ...
  sco: {
    ...
    lines: [
      [[0, 0], [15, -25], [30, -35]],
      [[0, 0], [0, -30], [-15, -40]],
      [[0, 0], [-10, 20], [-15, 40], [-5, 60], [15, 70], [30, 60], [25, 48]]
    ]
  },
  tau: {
    symbol: "♉",
  ...
  gem: {
    ...
    lines: [
      [[-15, -40], [-15, -10], [-20, 20], [-25, 45]],
      [[15, -35], [15, -5], [10, 25], [5, 50]],
      [[-15, -10], [15, -5]],
      [[-15, -40], [-35, -30]],
      [[15, -35], [35, -25]]
    ]
  ...
  peg: {
    symbol: "🐎",
    title: "Pegasus • Winged Horse",
    color: "rgba(180, 220, 255, 0.9)",
    lines: [
      [[-30, -30], [30, -30], [30, 30], [-30, 30], [-30, -30]],
      [[30, -30], [55, -45], [70, -35]],
      [[-30, -30], [-50, -60], [-70, -75], [-40, -45]],
      [[30, 30], [45, 60]],
      [[-30, 30], [-45, 60]]
    ]
  },
  cyg: {
    symbol: "🦢",
    title: "Cygnus • Celestial Swan",
    color: "rgba(150, 240, 255, 0.9)",
    lines: [
      [[0, -45], [0, -15], [0, 15], [0, 50]],
      [[-55, -5], [-25, -10], [0, -15], [25, -10], [55, -5]],
      [[-55, -5], [-75, -20]],
      [[55, -5], [75, -20]]
    ]
  },
  cas: {
    symbol: "👑",
    title: "Cassiopeia • Queen's Throne",
    color: "rgba(255, 215, 100, 0.9)",
    lines: [
      [[-40, 15], [-20, -20], [0, 5], [20, -25], [40, 10]],
      [[-40, 15], [-35, 40], [35, 40], [40, 10]],
      [[-20, -20], [-25, -40], [25, -40], [20, -25]]
    ]
  },
  sgr: {
    symbol: "♐",
    ...
    lines: [
      [[-20, 20], [0, 20], [15, 0], [0, -20], [-20, -20], [-20, 20]],
      [[15, 0], [35, 10], [45, -5]],
      [[-20, -20], [-35, -10], [-20, 20]],
      [[35, -40], [35, 0], [35, 40]]
    ]
  ...
  aql: {
    symbol: "🦅",
    title: "Aquila • Soaring Eagle",
    color: "rgba(240, 220, 120, 0.9)",
    lines: [
      [[0, -35], [0, 0], [0, 35]],
      [[-50, -15], [-25, -5], [0, 0], [25, -5], [50, -15]],
      [[0, 35], [-15, 55], [15, 55], [0, 35]]
    ]
  },
  cma: {
    symbol: "🐕",
  ...
  and: {
    symbol: "💃",
    title: "Andromeda • The Maiden",
    color: "rgba(255, 180, 220, 0.9)",
    lines: [
      [[0, -35], [0, -10], [0, 25], [10, 55]],
      [[-30, -20], [0, -10], [30, -20]],
      [[0, 25], [-20, 50]]
    ]
  },
  ...
  psc: {
    symbol: "♓",
    title: "Pisces • Two Fishes",
    color: "rgba(130, 220, 255, 0.9)",
    lines: [
      [[-40, -30], [-20, -10], [0, 10], [25, 35]],
      [[-40, -30], [-55, -40], [-45, -50], [-40, -30]],
      [[25, 35], [40, 45], [35, 55], [25, 35]]
    ]
  },
  aqr: {
    symbol: "♒",
  ...
  cap: {
    symbol: "♑",
    title: "Capricornus • Sea-Goat",
    color: "rgba(200, 230, 150, 0.9)",
    lines: [
      [[-40, -20], [-10, -30], [25, -15], [40, 10], [15, 30], [-20, 20], [-40, -20]],
      [[-10, -30], [5, -50]]
    ]
  },
  cnc: {
  ...
  lbr: {
    ...
    color: "rgba(240, 230, 150, 0.9)",
    lines: [
      [[-35, -20], [0, -30], [35, -20]],
      [[-35, -20], [-45, 15], [-25, 15], [-35, -20]],
      [[35, -20], [25, 20], [45, 20], [35, -20]]
    ]
  },
  lyr: {
    symbol: "🎻",
    title: "Lyra • Heavenly Harp",
    color: "rgba(180, 200, 255, 0.9)",
    lines: [
      [[0, -30], [-20, -10], [-15, 25], [15, 25], [20, -10], [0, -30]],
      [[-10, -10], [-10, 25]],
      [[0, -10], [0, 25]],
      [[10, -10], [10, 25]]
    ]
  ...
  aur: {
    symbol: "🛡️",
    title: "Auriga • Charioteer",
    color: "rgba(255, 220, 150, 0.9)",
    lines: [
      [[0, -35], [-30, -10], [-25, 30], [20, 35], [30, 0], [0, -35]],
      [[-30, -10], [-45, 0]]
    ]
  },
  oph: {
    symbol: "🐍",
    title: "Ophiuchus • Serpent Bearer",
    color: "rgba(160, 240, 200, 0.9)",
    lines: [
      [[0, -45], [-30, -20], [-25, 30], [25, 30], [30, -20], [0, -45]],
      [[-60, -10], [-30, -20]],
      [[30, -20], [60, -10]]
    ]
  },
  cru: {
    symbol: "✝️",
    title: "Crux • Southern Cross",
    color: "rgba(100, 240, 255, 0.95)",
    lines: [
      [[0, -35], [0, 35]],
      [[-25, -5], [25, -5]],
      [[0, -35], [25, -5], [0, 35], [-25, -5], [0, -35]]
    ]
  },
  car: {
    symbol: "⛵",
    title: "Carina • Ship's Keel",
    color: "rgba(120, 220, 255, 0.9)",
    lines: [
      [[-50, 10], [0, 30], [50, 10], [30, -20], [-30, -20], [-50, 10]],
      [[0, -20], [0, -50], [25, -35]]
    ]
  },
  cen: {
    symbol: "🐴",
    title: "Centaurus • Mighty Centaur",
    color: "rgba(255, 200, 150, 0.9)",
    lines: [
      [[35, 10], [15, -20], [-10, -20], [-35, 0], [-25, 30], [15, 30], [35, 10]],
      [[15, -20], [25, -50]],
      [[-35, 0], [-50, 20]]
    ]
  }
...
function drawAdvancedLayers() {
  ...
  if ((skySettings.showConstellationArt || skySettings.showConstellationArtwork) && typeof CONSTELLATION_FEATURES !== "undefined" && CONSTELLATION_FEATURES.length > 0) {
    context.save();
    for (const c of CONSTELLATION_FEATURES) {
      if (!c.geometry || !c.geometry.coordinates) continue;
      const raDec = [c.geometry.coordinates[0], c.geometry.coordinates[1]];
      if (Celestial.clip(raDec)) {
        const pt = Celestial.mapProjection(raDec);
        if (pt && !isNaN(pt[0]) && !isNaN(pt[1])) {
          const code = String(c.id || "").toLowerCase();
          const artDef = CONSTELLATION_ART_DEFINITIONS[code] || {
            symbol: "✨",
            title: c.properties?.name || c.id.toUpperCase(),
            color: "rgba(0, 245, 255, 0.85)",
            lines: [
              [[-30, -20], [0, -35], [30, -20], [20, 20], [-20, 20], [-30, -20]],
              [[-30, -20], [30, 20]],
              [[30, -20], [-20, 20]]
            ]
          };
          ...
          context.shadowColor = artDef.color || "rgba(0, 245, 255, 0.8)";
          context.strokeStyle = artDef.color || "rgba(0, 245, 255, 0.65)";
          context.lineWidth = 1.6;

          // Draw artistic mythological figure geometry
          if (artDef.lines && artDef.lines.length > 0) {
            for (const pathSegment of artDef.lines) {
              if (!pathSegment || pathSegment.length < 2) continue;
              context.beginPath();
              context.moveTo(pt[0] + pathSegment[0][0], pt[1] + pathSegment[0][1]);
              for (let i = 1; i < pathSegment.length; i++) {
                context.lineTo(pt[0] + pathSegment[i][0], pt[1] + pathSegment[i][1]);
              }
              context.stroke();
            }
          }

          // Draw glowing artistic emblem badge
          const badgeWidth = Math.max(90, (artDef.title.length * 7) + 26);
          const badgeHeight = 24;
          const bx = pt[0] - badgeWidth / 2;
          const by = pt[1] - badgeHeight / 2;

          context.fillStyle = "rgba(4, 16, 32, 0.75)";
          context.strokeStyle = artDef.color || "rgba(0, 245, 255, 0.75)";
          context.lineWidth = 1.2;

          context.beginPath();
          if (typeof context.roundRect === "function") {
            context.roundRect(bx, by, badgeWidth, badgeHeight, 12);
          } else {
            context.rect(bx, by, badgeWidth, badgeHeight);
          }
          context.fill();
          context.stroke();

          // Mythological Symbol & Title
          context.shadowBlur = 4;
          context.fillStyle = "#ffffff";
          context.font = "bold 11px 'Space Grotesk', sans-serif";
          context.textAlign = "center";
          context.textBaseline = "middle";
          context.fillText(`${artDef.symbol || "✨"} ${artDef.title || c.properties?.name || c.id.toUpperCase()}`, pt[0], pt[1]);
